...
<template>
  <div class="comunidades-page">
    ...
    <input v-model="searchQuery" type="text" placeholder="🔍 Buscar tecnología..." class="search-input" />
    <div class="container">
      <ComunidadCard v-for="c in comunidadesFiltradas" :key="c.id" :comunidad="c" />
    </div>
  </div>
</template>

<style scoped>
.comunidades-page { padding: 40px; text-align: center; }
.search-input { width: 100%; max-width: 500px; padding: 15px; border-radius: 25px; border: 2px solid #ddd; margin-bottom: 30px; font-size: 1rem; }
